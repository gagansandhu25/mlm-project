 ...
 * command — not per order. Logged to its own `personal_volume_accruals`
 * table rather than `commissions`: it isn't a network-tree payout (no
 * from_user/level/position), and a dedicated table lets us enforce one
 * accrual per user per day at the database level. Runs alongside
 * whichever network plan (unilevel/binary/matrix) is active. Gated by
 * the `personal_volume_commission_enabled` system setting.
 */
